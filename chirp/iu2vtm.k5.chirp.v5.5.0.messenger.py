...
DRIVER_VERSION = "Quansheng UV-K5/K6/5R driver ver: 2025/06/27 (c) EGZUMER + F4HWN v4.1.0"
...
// --------------------------------------------------------------------------
// Messenger (kamilsss655 port) - K5 EEPROM addresses
//
// MESSENGER_CONFIG lives in byte 3 of the 0x0EA0 settings block (= 0x0EA3),
// just after voice/s0_level/s9_level. Bitfield order matches the firmware
// MessengerConfig union (CHIRP declares MSB-first, so the listed order maps
// to bit7..bit0): receive=bit0, ack=bit1, encrypt=bit2, modulation=bit4..5.
// This mirrors the K1 driver's MESSENGER_CONFIG exactly.
#seekto 0x0EA3;
u8 __MSG_UNUSED1:2,
   msg_modulation:2,
   __MSG_UNUSED2:1,
...
// Encryption password (ENC_KEY[16]) at 0x0F30 on the K5 EEPROM. Same on-radio
// format as the kamilsss655 K5 menu (10 chars padded with '_', zero filled),
// so K1 and K5 derive the same 256-bit ChaCha20 key from the same password.
#seekto 0x0F30;
char enc_key[16];
...
# Messenger FSK modulation (kamilsss655 messenger port) - must match the K1
# driver so MsgMod values are identical across both radios
...
@directory.register
class IU2VTMRadioK5(chirp_common.CloneModeRadio):
    """Quansheng UV-K5 (iu2vtm Custom + Messenger)"""
    VENDOR = "Quansheng"
    MODEL = "UV-K5 (iu2vtm Custom)"
    BAUD_RATE = 38400
...
# bugfix calibration : put in comment next line: upload_calibration = False
    ...
    def set_settings(self, settings):
        ...
        for element in settings:
            ...
            # messenger (kamilsss655 port) - same handling as the K1 driver
...
# end add menu firmware with version and option display
        ...
        # Messenger (kamilsss655 port) - identical settings to the K1 driver so
        # the configuration procedure is the same on both radios. Always shown
        # (this firmware always includes the messenger).
...
#
        ...
        # Messenger (kamilsss655 port) - mirrors the K1 driver. Always shown.
        append_label(basic,
                     "=" * 6 + " Messenger " + "=" * 300, "=" * 300)
